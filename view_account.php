...
<div class = "main-body">
	...
	<?php 
		...
		$ulink = mysqli_connect($host,$user,$password,$db);
		$uconnect = new PDO("mysql:host=localhost;dbname=paathshala;charset=utf8mb4", "root", "");

		//data query
		
		$usql = 'SELECT * FROM user WHERE id = "'.$u_id.'"';
		$uresult = mysqli_query($ulink,$usql) or die(mysqli_error($ulink));
		...
		while($urow = mysqli_fetch_row($uresult)){
			...
			$urating=$urow[6];
	?>
		<img src="assets/images/circle.gif" alt="Logo" style="width:15%;"><br>
		<p class="ac_name"><?php echo $uname; ?></p>
		<p class = "ac_name text-primary"><?php echo $uemail; ?></p>
		<div class="tags">
			<?php if($urating>500) { ?>
			<button class="p-tag">TEACHER</button>
			<?php }elseif($urating>300) { ?>
			<button class="p-tag">ASSISTANT</button>
			<?php }elseif($urating>200) { ?>
			<button class="p-tag">AMATEUR</button>
			<?php }elseif($urating>100) { ?>
		...
		<div class= "rating">
			...
			<span class="p-head"><?php echo $urating; ?></span>
		</div>
		<?php } ?>
	</div>
</div>
